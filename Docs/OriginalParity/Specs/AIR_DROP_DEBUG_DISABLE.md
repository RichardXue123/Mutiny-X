...
> **历史状态（已结束）**：2026-09-17 按用户要求恢复空投。当前生产规则、实现和验收见 `AIR_DROP.md`；本文件只保留临时停用的授权与追踪记录。

## 范围与授权

这是用户于 2026-09-16 明确授权、后于 2026-09-17 明确结束的临时调试扩展，不代表原版规则变化。原版空投行为由 P6-01～P6-07 和 `AIR_DROP.md` 跟踪。
...
恢复工作已重新核对原版触发（无独立概率掷骰）、最多数量、有效列、下降、拾取时序、镜头、声音和 AI 评分。Unity Play Mode 验收仍以 `AIR_DROP.md` 为准。
