...
| 用例 | 操作与断言 | 状态 |
| --- | --- | --- |
| ROT-TC01 | 通过 `MutinyPhysicsBody.AdvanceSimulationTick` 驱动角色，X 速度为 4 时单 tick 为 -12°；连续 tick 只随逻辑 tick 数增长。 | 待自动验证 |
| ROT-TC02 | 角色以 270° 接触地面后得到 -45°；2° 得到 1°，后续接触归零。 | 待自动验证 |
...
- 原版 Boulder 旋转 `mc.rotating` 子层。当前 Unity Boulder 只有单一精灵渲染层，因此在等价的武器根 Transform 上旋转；若后续重建 Boulder 嵌套时间轴，应把相同规则迁移到对应子层。
