...
| --- | --- | --- | --- |
| Cherry Bomb | 4 帧弹体循环 + 每 tick 烟迹 + 80/40 explosion | `MutinyCherryBomb` 循环 4 帧，碰撞生成 explosion | 主体/爆炸已实现；烟迹缺失 |
| Dynamite | `lit` 可见帧 1..4 循环，frame 5 跳回；入水停在 `unlit` frame 6；每 tick 烟迹 | `MutinyDynamite` 以生产物理 tick 推进 lit 帧 | ready 动画本轮修复；入水 dud 仍是逻辑差异 |
...
| Cannon / Cannonball | 炮身、pin、range circle 分离；炮弹在炮身上层并留烟 | `MutinyCannon` 分层组件；`MutinyCannonball` sortingOrder+1 | 层级/炮弹已实现；烟迹资源缺失 |
...
| Parachute Bomb | closed 外层停止但四帧引信子时间轴循环；未开伞时留烟；opening 11..29；30 跳回 open 26 | `MutinyParachuteBomb` 分开引信与开伞推进 | 帧序已实现；烟迹资源缺失；待运行验证 |
| Rum Bottle | 12 帧瓶体循环并留烟；地面命中生成双向 sweepingFlame | `MutinyRumBottle` + `MutinySweepingFlame` | 帧序/火焰已实现；烟迹资源缺失；待运行验证 |
