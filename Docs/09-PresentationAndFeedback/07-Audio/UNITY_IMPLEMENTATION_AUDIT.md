...
### U-AUD-003 · 静音跨状态后恢复错误曲目

`PlayMusic` 在 `MusicEnabled == false` 时立即返回，既不换 clip，也不记录逻辑音乐类型。例如战斗音乐播放中关闭音乐、返回菜单、再开启时，Unity 会重播战斗音乐；原版会在静音期间记录 `music_type="menu"`，重新开启时播放菜单音乐。

建议：把“期望曲目”与“当前是否发声”分离。`RequestMusic(Menu/Game)` 永远更新逻辑状态；只有开关开启时才操作 `AudioSource`。
...
3. 修复静音期间的逻辑曲目记录；
4. 删除或隔离结算与武器发射的非原版声音；
5. 建立队伍语音映射和音量包络；
6. 接入前导时间轴（若 Unity 产品范围包含该流程）；
7. 增加缺失资源、音乐状态和实际播放诊断。
