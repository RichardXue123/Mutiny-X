...
- `MutinyBoulder.ApplyCharacterContacts`：巨石有横向撞击速度时，每次新的角色接触播放原版导出的 `smack`；静止重叠静默，同一角色持续重叠不重复，同 tick 多目标合并为一次，离开后重入可再次播放。原版 `Boulder.as`、pcode 和 symbol 872/869 没有此调用，故登记为 `BLD-AUD-EXT-01` / `AUD-EXT-BLD-001`，不作为原版一致性通过项。
