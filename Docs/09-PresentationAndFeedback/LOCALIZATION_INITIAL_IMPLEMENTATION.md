...
- `MutinyLocalization` 缓存两个表，切换时保存偏好并发送变更事件；单条缺译回退到英语，初始表未就绪时使用调用点英语，缺键只警告一次。`MutinySaveSystem` 的语言键独立于进度。未来增加 `ja`、`zh-Hant` 时，需在语言目录、选择 UI 中增加代码和相应表，不改武器、关卡等业务 ID。
...
- 已接入标题页语言按钮、前端主要导航/说明/成绩/制作页、载入文字、战斗武器名和说明、回合/拾取提示、退出和结算弹窗。武器内部 ID 不翻译。事件队列保留待解析文案，切换语言后未显示及当前显示的提示会按新语言解析，且不重新开始其动画计时。
...
| LOC-02 当前页切换 | 用户授权；原版菜单时间轴 | `MutinyFrontendController.DrawTitle`、`DrawOriginalButton` | 已实现主要导航 | 鼠标/触控及各页切换的画面验收待做 |
| LOC-03 英语外观 | 原版 `PirateFont`、`DangleFont` 图集和 SWF 帧脚本 | `MutinyLocalizedText` → `MutinyBitmapFont` | 已保留绘制路径 | 截图逐态对比待做；新增语言按钮改变标题页布局 |
...
- **已知差异：** 原版武器面板空闲标题为 `Weapons`，目前 Unity 实现和英语表为 `weapons`；沿用既有差异，未在本地化任务中改变。贴图英语和对话未完成，简中字号/溢出尚需实机调整。初版语言按钮位于原版标题页按钮下方，为授权扩展。
