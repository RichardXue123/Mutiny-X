...
| LOC-05 船长对话 | SWF `Team.as::lines`、结尾 DangleFont 构造脚本 | `MutinySpeechController`、`MutinyGameHUD`、`MutinyFrontendController.DrawEnding` | 60 条战斗、5 段结尾已翻译；中文 15px 深色动态字体 | 65 段缺字/排版检查通过；对白及角落提示生产入口 124/124 通过，详见补充记录 |
...
- **已知差异：** 原版武器面板空闲标题为 `Weapons`，目前 Unity 实现和英语表为 `weapons`；沿用既有差异，未在本地化任务中改变。初版遗漏的对白及角落提示已由补充实施覆盖，剩余贴图英语仍需审计；简中实际画面与真机缩放待验收。初版曾在标题页增加语言按钮，已由本轮仅保留 GM 入口的规格替代。
