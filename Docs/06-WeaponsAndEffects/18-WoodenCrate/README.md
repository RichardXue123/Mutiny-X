...
| BOX-WAIT-01 | 第一/第二箱后可无限等待下一次玩家点击；原版没有 150 tick/6 秒强制结束，非法点击和等待都不能销毁箱链 | `BoxWeapon.as::advance/place`；无超时分支 | `MutinyTurnManager.AdvanceSimulationTick` | 第一箱后驱动 `MutinyTurnManager.AdvanceSimulationTick()` 超过 150 tick，断言箱链仍 pending 且后续两次合法点击可完成 3 箱 | 已实现；待 Unity 运行验证 |
