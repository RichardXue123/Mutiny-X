...
原版来源：不适用，用户授权扩展。`setlanguage cn` / `setlanguage en` 经 `ExecuteCommand` 调用本地化服务，菜单和战斗共用，成功后保存语言并加入最近命令。拒绝缺参、非法语言及额外参数，原选择及历史保持不变。详细规格和验收见 [GM-09](../GM_COMMANDS.md#gm-09--切换语言)；已实现，2026-09-26 隔离 Unity Play Mode 通过 **7/7** 生产入口断言。重启和实际面板输入仍待验收。
