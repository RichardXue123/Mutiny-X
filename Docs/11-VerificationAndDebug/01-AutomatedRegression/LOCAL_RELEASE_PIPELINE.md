...
| EXT-REL-05 | 发布先创建草稿，上传指定资产并验证大小和 GitHub SHA256；完整验证后转为正式 Release；已发布 tag 拒绝覆盖 | ReleaseTools.psm1 | 以 GitHub CLI 替身驱动真实发布逻辑，验证草稿→上传→校验→发布的顺序，以及失败不发布 | 已实现；CLI 替身回归通过，首次真实线上发布待运行 |
| EXT-REL-06 | `-Resume` 仅复用同 tag/提交/平台且 SHA256 匹配的完整 manifest；重复上传可跳过匹配资产；只操作可核验身份的草稿 | Publish-Release.ps1 / ReleaseTools.psm1 | 中断上传后恢复、文件被改动、草稿标题/说明/提交不匹配、额外资产 | 已实现；真实本地产物恢复及 CLI 替身恢复通过 |
...
- 本机尚未执行 `gh auth login`；首次真实 GitHub 草稿创建、上传与发布需在新的正式版本 tag 下运行。模拟结果只证明编排和失败处理，不代表本次进行了真实线上发布。
...
- 本次仅修复前置检查和使用说明；此前真实 Unity / Inno 构建记录继续适用，未重复打包，未创建新正式 tag 或 Release。准备新版本的提交、tag 和首次真实线上发布仍待执行。
...
- 后续：将发布源码提交后更新 tag，再执行真实构建和发布；结果另行记录。既有游戏代码不因版本修复改变。
