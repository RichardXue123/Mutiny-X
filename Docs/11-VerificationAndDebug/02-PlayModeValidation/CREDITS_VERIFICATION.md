...
2026-09-26，从 main 建立独立发布工作区，纳入 Credits 资源修复及 GM-08；未纳入正在开发的手柄输入代码。Unity 6000.6.0f1 再次执行 `MutinyCreditsVerificationMenu.Validate()` 通过。Main 场景 Play Mode 经 Credits 按钮共用的 `OpenCredits` 入口和实际过渡进入页面，使用 Game View 的实际渲染目标截图；人工检查截图，头像、头像下方 Richard Xue 和中央移植说明均可见。

![EXT-CRED-01 1.0.1 编辑器 Credits 实际画面](Artifacts/EXT-CRED-01-v1.0.1-editor.png)

本轮编辑器视觉验收已通过；上述 Windows 隐藏窗口黑屏记录保留为历史失败尝试。实际鼠标外链点击、移动端真机显示及 Player 可见窗口截图仍待验证。Development Player 截图辅助现等待 Unity SplashScreen 完成后再开始，避免启动画面影响取证。
